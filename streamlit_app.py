    ...
    if len(tax_id) == 13 and api_key:
        ...
        with st.spinner("🔍 AI กำลังสืบค้นข้อมูลบริษัทจากเลข 13 หลัก..."):
            try:
                ...
                search_prompt = f"""
                ใช้ Google Search เพื่อค้นหาข้อมูลนิติบุคคลไทยจากเลขประจำตัวผู้เสียภาษี 13 หลักนี้: {tax_id}
                จงตรวจสอบว่าเลขนี้คือชื่อบริษัท/ห้างหุ้นส่วนอะไร และทำธุรกิจเกี่ยวกับอะไร
                
                ตอบกลับเป็นรูปแบบ JSON เท่านั้น ห้ามมีคำอธิบายอื่น โครงสร้างดังนี้:
                {{
                  "company_name": "ชื่อบริษัทภาษาไทย (ระบุ ประเภทบริษัท/หจก. ด้วย)",ห
                  "business_type": "สรุปประเภทธุรกิจสั้นๆ (เช่น ผลิตเบเกอรี่, ขายปลีกเสื้อผ้า)"
                }}
                """
